...
const SelectPage = () => {
  const { checkedValue, setCheckedValue } = useContext(SelectPlanContext); // Default to 'monthly'
  ...
  return (
    <>
      <div className="sm:flex mainOuterDiv border sm:justify-center sm:py-16 pb-10 bg-Background ">
        <div className="mainInnerdiv bg-Background sm:p-5  sm:justify-center sm:flex rounded-xl bg-white">
          ...
          <div className="sm:border-0 border rounded-xl  sm:px-24 px-7 py-10 sm:w-full w-96 sm:relative absolute top-24 left-32 sm:top-0 sm:left-0 bg-white ">
            ...
            <div className="flex sm:justify-around justify-between w-full absolute  sm:right-0 top-96 sm:top-[31rem] top-[35rem]  ">
              ...
              <button className="bg-marine-blue hover:bg-Denim text-white font-bold py-3 px-6 rounded-lg" onClick={() => navigate("/pick-one")}>Next Step</button>
            </div>
          </div>
        </div>
      </div>
